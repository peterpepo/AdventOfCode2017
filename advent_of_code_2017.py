RESULT_PRINT_FORMAT = "Day {day_number}, part A: {solution[0]}\nDay {day_number}, part B: {solution[1]}"

# Run day_01
from day_01 import day_01
print(RESULT_PRINT_FORMAT.format(day_number = "01", solution = day_01.solve()))

# Run day_02
from day_02 import day_02
print(RESULT_PRINT_FORMAT.format(day_number = "02", solution = day_02.solve()))

# Run day_03
from day_03 import day_03
print(RESULT_PRINT_FORMAT.format(day_number = "03", solution = day_03.solve()))

# Run day_04
from day_04 import day_04
print(RESULT_PRINT_FORMAT.format(day_number = "04", solution = day_04.solve()))

# Run day_05
from day_05 import day_05
print(RESULT_PRINT_FORMAT.format(day_number = "05", solution = day_05.solve()))

# Run day_06
from day_06 import day_06
print(RESULT_PRINT_FORMAT.format(day_number = "06", solution = day_06.solve()))

# Run day_08
from day_08 import day_08
print(RESULT_PRINT_FORMAT.format(day_number = "08", solution = day_08.solve()))

# Run day_10
from day_09 import day_09
print(RESULT_PRINT_FORMAT.format(day_number = "09", solution = day_09.solve()))

# Run day_10
from day_10 import day_10
print(RESULT_PRINT_FORMAT.format(day_number = "10", solution = day_10.solve()))

# Run day_11
from day_11 import day_11
print(RESULT_PRINT_FORMAT.format(day_number = "11", solution = day_11.solve()))

# Run day_13
from day_13 import day_13
print(RESULT_PRINT_FORMAT.format(day_number = "13", solution = day_13.solve()))

# Run day_14
from day_14 import day_14
print(RESULT_PRINT_FORMAT.format(day_number = "14", solution = day_14.solve()))

# Run day_16
from day_16 import day_16
print(RESULT_PRINT_FORMAT.format(day_number = "16", solution = day_16.solve()))

# Run day_17
from day_17 import day_17
print(RESULT_PRINT_FORMAT.format(day_number = "17", solution = day_17.solve()))